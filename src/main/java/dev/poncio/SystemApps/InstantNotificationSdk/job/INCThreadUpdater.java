...
public class INCThreadUpdater extends Thread {
    ...
    private BlockingQueue<INCJobUpdate> jobs2Sync = new ArrayBlockingQueue<>(Integer.MAX_VALUE);
    ...
    public void run() {
        while (true) {
            ...
            try {
                ...
                ResponseEntity result = httpService.makeReq(MetodoRequisicao.POST,
                        brokerConfiguration.getUrl() + "/job/update/" + update.getJob().getId(), update, null, null);
                if (result == null) {
